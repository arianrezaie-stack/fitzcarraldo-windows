...
function Home() {
  ...
  const inputChannelCount = Math.max(1, Math.min(8, selectedPair?.input.channels || 8));
  const outputChannelCount = Math.max(1, Math.min(8, selectedPair?.output.channels || 8));
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      <section className="hero-panel"><div className="hero-grid"><div><div className="status-row"><Badge tone={audioRunning ? 'green' : 'red'}>{audioRunning ? <><Radio size={12} /> routing running</> : <><Power size={12} /> routing stopped</>}</Badge><span className="route-label">{telemetry.calibrating ? 'Calibration sweep in progress' : telemetry.protectionEnabled ? `${telemetry.preset ?? preset} protection active` : 'Protection bypassed'}</span></div><div className="hero-copy"><div><h2 className="hero-title">Measured acoustic protection</h2><p className="hero-description">Calibrate the loudspeaker loop, then use bounded adaptive notch filters to control persistent feedback while preserving speech or music.</p></div><div className="hero-actions"><button type="button" className={`action-button ${audioRunning ? 'off' : ''}`} disabled={!nativeReady || (!audioRunning && !selectedPair)} onClick={audioRunning ? stop : configureAndStart}>{audioRunning ? <><Power size={15} /> Stop</> : <><Zap size={15} /> Configure &amp; Start</>}</button></div></div>
       ...
       <div className="panel"><div className="section-kicker"><LockKeyhole size={14} /> Route {activeRoute + 1} audio backend &amp; engine</div><label className="device-select"><span><span className="device-name">{selectedPair ? `${selectedPair.input.name} → ${selectedPair.output.name}` : 'No compatible input/output pair'}</span><span className="device-detail">{selectedPair ? `${backendLabel(selectedPair.input.deviceType)} · shared physical device` : 'Awaiting native audio device records'} · requested 48 kHz</span></span><select value={selection} disabled={!nativeReady || audioRunning || !pairs.length} onChange={(event) => setSelection(event.target.value)} aria-label="Shared audio device pair">{!pairs.length && <option value="">No compatible audio devices</option>}{pairs.map((pair) => <option key={pair.key} value={pair.key}>{backendLabel(pair.input.deviceType)} · {pair.input.name} → {pair.output.name}</option>)}</select></label><div className="route-bus-grid"><label><span>Route {activeRoute + 1} mono input → output pair</span><select value={`${activeRouteState?.inputChannel ?? 0}:${activeRouteState?.outputChannel ?? 0}`} disabled={!nativeReady || audioRunning} onChange={(event) => { const [inputChannel, outputChannel] = event.target.value.split(':').map(Number); updateRouteChannel(activeRoute + 1, 'inputChannel', inputChannel); updateRouteChannel(activeRoute + 1, 'outputChannel', outputChannel); }}>{monoRoutePairs.map((pair) => <option key={pair.key} value={pair.key}>Input {pair.inputChannel + 1} → Output {pair.outputChannel + 1}</option>)}</select></label></div><div className="detail-row"><span>Selected backend</span><strong>{selectedPair ? backendLabel(selectedPair.input.deviceType) : '—'}</strong></div><div className="detail-row"><span>Route {activeRoute + 1} mapping</span><strong>Input {(activeRouteState?.inputChannel ?? 0) + 1} → Output {(activeRouteState?.outputChannel ?? 0) + 1}</strong></div><div className="detail-row"><span>Requested buffer</span><select value={buffer} disabled={!nativeReady || audioRunning} onChange={(event) => setBuffer(event.target.value)}><option value="64">64 samples</option><option value="128">128 samples</option><option value="256">256 samples</option></select></div><div className="detail-row"><span>Engine state</span><strong>{audioState.phase ?? engineStatus.state}</strong></div><div className="health-grid"><div className="health"><label>Sample rate</label><strong>{rate === undefined ? '—' : `${rate} Hz`}</strong></div><div className="health"><label>Buffer</label><strong>{actualBuffer === undefined ? '—' : `${actualBuffer} smp`}</strong></div><div className="health"><label>Input peak</label><strong>{peakPercent(telemetry.inputPeak) === undefined ? '—' : `${value(peakPercent(telemetry.inputPeak))}%`}</strong></div><div className="health"><label>Output peak</label><strong>{peakPercent(telemetry.outputPeak) === undefined ? '—' : `${value(peakPercent(telemetry.outputPeak))}%`}</strong></div></div><p className="section-note">The physical backend stays shared so all routes use one stable device clock. Each route independently selects one mono input-to-output bus pair. Stereo buses are never sent to the route processor.</p></div></div></section>

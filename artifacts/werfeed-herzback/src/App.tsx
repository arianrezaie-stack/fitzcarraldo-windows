    ...
    <main className="main-content section-stack">
      ...
      <section className="panel calibration-panel">
       ...
       <section className="route-focus-stack">
          <section className="panel route-analyzer-panel"><div className="panel-heading"><div><div className="section-kicker"><Radio size={14} /> route {activeRoute + 1} analyzer</div><h3 className="section-title">Live spectrum and adaptive cuts</h3><p className="section-note">The detector uses a virtual flat reference until calibration adds measured room weighting. Armed routes share 24 adaptive notch slots; disarmed routes return their share to the remaining routes.</p></div><span className="slot-count">{activeNotches?.length ?? 0} / {activeNotchCapacity} cuts</span></div><Spectrum values={activeSpectrum} notches={activeNotches} />{recurringCutAlert?.route === activeRoute + 1 && <div className="recurring-cut-alert" role="status" aria-live="polite"><span>Consider adding a system cut at <strong>{recurringCutAlert.frequency.toLocaleString('en-US', { maximumFractionDigits: 0 })} Hz</strong>. This frequency keeps returning.</span><button type="button" className="recurring-cut-dismiss" onClick={dismissRecurringCutAlert} aria-label={`Reassess recurring ${Math.round(recurringCutAlert.frequency)} Hz warning`} title="Reassess this frequency"><X size={15} /></button></div>}</section>
